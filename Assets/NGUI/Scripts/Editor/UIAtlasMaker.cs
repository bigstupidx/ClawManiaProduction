...
public class UIAtlasMaker : EditorWindow
{
	...
	void OnGUI ()
	{
		...
		auto_width = EditorGUILayout.IntField ("Width", auto_width, GUILayout.Width (500f));
		auto_height = EditorGUILayout.IntField ("Height", auto_height, GUILayout.Width (500f));
		auto_amount_x = EditorGUILayout.IntField ("Amount X", auto_amount_x, GUILayout.Width (500f));
		auto_amount_y = EditorGUILayout.IntField ("Amount Y", auto_amount_y, GUILayout.Width (500f));
		max_index = EditorGUILayout.IntField ("Amount Y", max_index, GUILayout.Width (500f));
		GUILayout.BeginHorizontal();
		if (GUILayout.Button("Auto Generated Sprites"))
		{
			NGUISettings.atlas.spriteList = new List<UISpriteData>();
			int index = 0;
			for ( int i=0; i<auto_amount_y; i++ )
			{
				for ( int j=0; j<auto_amount_x; j++ )
				{
					UISpriteData data = new UISpriteData();
					data.width = auto_width;
					data.height = auto_height;
					data.x = j*auto_width;
					data.y = i*auto_height;
					data.name = "Animation"+index;
					NGUISettings.atlas.spriteList.Add(data);
					index++;
					if ( index >= max_index )
						break;
				}
			}
			Debug.LogError(NGUISettings.atlas.spriteList.Count);
		}
		GUILayout.EndHorizontal();
