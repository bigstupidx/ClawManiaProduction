...
public class UISpriteAnimation : MonoBehaviour
{
	...
	{
		if (mActive && mSpriteNames.Count > 1) {
			mDelta += Time.deltaTime * 10;
			if ((int)mDelta >= mSpriteNames.Count) {
				mDelta = 0;
			}
			
			mSprite.spriteName = mSpriteNames[(int)mDelta];
		}
		/*
		if (mActive && mSpriteNames.Count > 1 && Application.isPlaying && mFPS > 0)
		{
			mDelta += Time.deltaTime * 0.4f;
			float rate = 1f / mFPS;
